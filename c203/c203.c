...
*/
	return ((q->f_index == q->b_index) ? 1 : 0); // ak je prvy prazdny zaroven aj zaciatkom
}
...
*/
	return ((nextIndex(q->b_index) == q->f_index) ? 1 : 0); // ak je plna fronta tak sa nasledujuci za poslednym (prvy prazdny) rovna prvemu vo fronte
}
